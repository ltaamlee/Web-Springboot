...
public class CategoryServiceImpl implements CategoryService {
	...
	public <S extends Category> S save(S entity) {
		if (entity.getCateId() == null) {
			...
			Optional<Category> opt = findById(entity.getCateId());
			if (opt.isPresent()) {
				if (StringUtils.isEmpty(entity.getImg())) {
					entity.setImg(opt.get().getImg());
				} else {
	...
	@Override
	public List<Category> findByCateNameContaining(String name) {
	    return categoryRepository.findByCateNameContaining(name);
	}

	@Override
	public Page<Category> findByCateNameContaining(String name, Pageable pageable) {
		return categoryRepository.findByCateNameContaining(name, pageable);
	}
